...
router.get(
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    const [totalUsers, activeSubscriptions, totalDraws, completedDraws] = await Promise.all([
    ...
    const totalCharityContributions = activeSubs.reduce((sum, s) => sum + s.price * (s.contributionPercent / 100), 0);
    // console.log(totalCharityContributions);
    ...
    return success(res, 200, {
      ...
      charity: {
        totalDonated: totalCharityContributions,
      },
      draws: {
