        ...
        return (
            <div className="h-screen bg-background flex flex-col items-center justify-center gap-8 selection:none">
                ...
                <div className="text-center space-y-2">
                    <p className="font-black italic text-xl uppercase tracking-tighter">Authenticating Authority</p>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.3em] italic">Verifying clearance level...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="flex flex-col h-full bg-foreground text-background relative selection:bg-primary/20">
            <div className="p-8 pb-12">
                <Link href="/" className="flex items-center gap-4 group">
                    ...
                    <div className="flex flex-col">
                        <span className="text-2xl font-black italic tracking-tighter leading-none">ROOT_ACCESS</span>
                        <span className="text-[9px] font-black text-primary uppercase tracking-[0.4em] mt-1.5 opacity-80">Authority Panel</span>
                    </div>
                </Link>
            </div>
            ...
            <nav className="flex-1 px-4 space-y-3 overflow-y-auto pb-8">
                <div className="px-5 mb-6">
                    <p className="text-[10px] font-black text-background/30 uppercase tracking-[0.4em] italic leading-none">System Nodes</p>
                </div>
                {adminItems.map((item) => {
                    ...
                    return (
                        <Link
                            ...
                            href={item.href}
                            className={cn(
                                "flex items-center gap-4 px-5 py-4 rounded-2xl text-sm font-black italic transition-all group relative overflow-hidden",
                                isActive
                        ...
                        >
                            <item.icon className={cn("h-5 w-5 transition-transform", !isActive && "group-hover:scale-110 group-hover:rotate-3")} />
                            <span className="uppercase tracking-tight">{item.name}</span>
                            {isActive && (
                                <Zap className="absolute right-5 h-4 w-4 text-primary fill-current animate-pulse" />
                            )}
                        </Link>
                    );
                })}
            ...
            <div className="p-6 mt-auto border-t border-background/10 space-y-6 bg-background/5">
                <div className="px-2">
                    <p className="text-[10px] font-black text-background/30 uppercase tracking-[0.4em] italic">Authority ID</p>
                </div>
                ...
                <div className="flex items-center gap-4 px-2">
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-sm font-black italic truncate tracking-tight text-background">{session?.user?.name || "Root Admin"}</p>
                        <Badge className="h-4 px-1.5 bg-primary text-primary-foreground border-none rounded-sm text-[8px] font-black tracking-widest uppercase italic mt-1 shadow-lg shadow-primary/20">Authorized Overlord</Badge>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-2 pt-2">
                    <Button
                        variant="ghost"
                        className="w-full justify-start gap-4 h-12 px-5 rounded-xl bg-background/5 text-background/60 hover:bg-background/10 hover:text-background transition-all text-[10px] font-black italic uppercase tracking-widest"
                        asChild
                    >
                        <Link href="/dashboard">
                    ...
                    <Button
                        variant="ghost"
                        className="w-full justify-start gap-4 h-12 px-5 rounded-xl bg-destructive/10 text-destructive hover:bg-destructive hover:text-destructive-foreground transition-all text-[10px] font-black italic uppercase tracking-widest"
                        onClick={() => signOut()}
                    >
            ...
            <div className="flex-1 flex flex-col min-w-0 relative">
                ...
                <header className="lg:hidden flex items-center justify-between px-6 h-20 border-b border-border bg-foreground text-background sticky top-0 z-30">
                    <Link href="/" className="flex items-center gap-3">
                        ...
                        <span className="text-xl font-black italic tracking-tighter uppercase leading-none">Admin_Terminal</span>
                    </Link>
                    <Button
                        variant="ghost"
                        size="icon"
                    ...
                    <div className="max-w-[1500px] mx-auto p-6 md:p-12 lg:p-16 xl:p-24 min-h-full flex flex-col">
                        ...
                        <div className="pt-32 pb-10 mt-auto flex flex-col md:flex-row items-center justify-between gap-8 opacity-20 hover:opacity-50 transition-opacity">
                            <div className="flex items-center gap-4">
                                ...
                                <p className="text-[9px] font-black italic uppercase tracking-[0.4em]">ROOT_NODE // REGISTRY_v2.0.46</p>
                            </div>
